...
subordinates[2..45].each do |office|
  ...
  if office.css('a')[0] != nil
    ...
    m = 0
    chief_info = []
    position = office.css('tr').css('p').css('.tel')
    while position[m] != nil && m <= 12
      if position[m].text.strip[0] != "(" && position[m].text.strip.index("Fax") != 0 && position[m].text.strip.index("TTY") != 0 && position[m].text.strip[0] != "" && position[m].text.strip != nil
